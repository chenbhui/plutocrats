...
<template>
    <div class="animation-list">
        <div class="div-animation">
            ...
            <div>
                <el-tag v-for="(tag, index) in curComponent.animations" :key="index" closable
        ...
        <Modal v-model="isShowAnimation">
            <el-tabs v-model="animationActiveName">
                <el-tab-pane v-for="item in animationClassData" :key="item.label" :label="item.label"
                    :name="item.label">
                    <el-scrollbar class="animate-container">
                        <div v-for="animate in item.children" :ref="animate.value" :key="animate.value" class="animate"
                            @mouseenter="runAnimation(animate)" @click="addAnimation(animate)">
                            <div>
                                {{ animate.label }}
                            </div>
                        </div>
        ...
        <!-- 编辑动画配置 -->
        <AnimationSettingModal v-if="isShowAnimationSetting" :cur-index="curIndex"
...
import AnimationSettingModal from '@/components/AnimationSettingModal'
...
export default {
    ...
    methods: {
        addAnimation(animate) {
            this.$store.commit('EditPage/addAnimation', animate)
        ...
        removeAnimation(index) {
            this.$store.commit('EditPage/removeAnimation', index)

<!-- 添加动画后对动画的设置 -->
<template>
    <el-dialog :title="`${config.label} 动画配置`" :visible="centerDialogVisible" width="30%" center
        @close="handleCloseModal">
        <div class="time">
            动画时长：
            <el-input-number v-model="config.animationTime" controls-position="right" :min="0.1" :precision="2"
                :step="0.01" />
        </div>
        <div class="loop">
...
export default {
    ...
    methods: {
        ...
        handleSaveSetting() {
            ...
            this.$store.commit('EditPage/alterAnimation', {
                index: this.curIndex,
                data: {
